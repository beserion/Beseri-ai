...
import json
import webbrowser
from core import WormAi as BeseriAi
...
def main():
    print_header()
    current_proxy = proxy
    client = BeseriAi(current_proxy)
    extra_data = None
    last_response = ""
    print("Beseri-Ai CLI — type your message and press Enter. Commands: /exit /restart /web /proxy <url>")
    print("[✓] Beseri-AI Ready.")
    while True:
        try:
            msg = input("> ").strip()
            if not msg:
            ...
            if msg == "/web":
                if not last_response:
                    ...
                    continue
                path = os.path.join(os.getcwd(), "beseriai_response.html")
